...
function postReducer(state=initState,{type,payload}) {
  switch (type) {
      ...
      case ADD_COMMENT:
          return state.map(post=>post.postId===payload.postId ? {...post,comments:[payload.formData,...post.comments]}:post)
      case REMOVE_COMMENT:
          return state.map(post=>post.postId===payload.postId ? {...post,comments:post.comments.filter(comment=>comment.commentId!==payload.commentId)}:post)
      default:
          return state
  }
}
